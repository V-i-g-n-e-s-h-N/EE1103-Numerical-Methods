...
Version 1
Descrption : This program stores the values generated by a Lorentzian or Gaussian function, with noise added, to a file. 
             It then reads from the file, 1 point at a time, and determines the times at which a peak in the amplitudes occurs. 
             Then the program determines the spacing between 2 peaks, and the width of each peak. 
             Additionally, the code uses an exponential filter to reduce the noise at each point. 
             
             
Input : This program takes 4 command line inputs from the user :
        a. M - the number of peaks in the Lorentzian time series. 
        b. T - the time spacing between 2 peaks in the time series. 
        c. a - the width of each curve, approximated using the FWHM(Full Width at Half Maximum) method.
        d. Type - gaussian/Gaussian - An input to see if the user wants Gaussian or Lorentzian curves to be used. By default, it is Lorentzian. 

        
Output : The program prints out a sequence of 4 integers :
        a. Average value of T
        b. Average value of a
        c. Stadard deviation in T
        d. Standard deviation in a
*/
...
int main(int argc, char** argv){
	...
	// Finding avg and standard deviations
...
// Function to generate random standard normal distribution value using Box-Muller Algorithm
